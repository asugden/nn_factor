...
from nn_factor.network_tools import (
    default_model,
    positional_encoding,
    transformer_nomlp,
)
...
class FactorModelCrossAttnNoMLP(default_model.DefaultModel):
    def __init__(
        self,
        feature_dim: int = 36,
        embed_dim: int = 64,
        ...
        x1 = self.add_pos(x1)
        x2 = self.add_pos(x2)
        self.selfattn = transformer_nomlp.TransformerNoMLPBlock(
            embed_dim=embed_dim,
            key_dim=embed_dim // 2,
        ...
        self.crossattn = transformer_nomlp.TransformerNoMLPBlock(
            embed_dim=embed_dim,
            key_dim=embed_dim,
            value_dim=embed_dim,
            num_heads=num_heads,
