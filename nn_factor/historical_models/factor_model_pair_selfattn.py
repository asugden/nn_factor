...
class FactorModelPairSelfAttn(default_model.DefaultModel):
    def __init__(
        ...
        # Inputs
        input = tf.keras.layers.Input(shape=(feature_dim,), dtype=np.int32)
        ...
        x1 = tf.keras.layers.Lambda(lambda v: v + self.pos_half)(x1)
        x2 = tf.keras.layers.Lambda(lambda v: v + self.pos_half)(x2)
        ...
        x = tf.keras.layers.Lambda(lambda v: v + self.pos_whole)(x)
