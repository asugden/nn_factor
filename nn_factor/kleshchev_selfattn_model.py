...
class KleshchevSelfAttnModel(default_model.DefaultModel):
    def __init__(
        ...
        if not k_segment_as_multiplicative:
            ...
            # dimension as X
            segment = tf.keras.layers.TimeDistributed(
                tf.keras.layers.Dense(embed_dim, activation=activation_fn),
            ...
            x = tf.keras.layers.LayerNormalization(epsilon=1e-6)(x + segment + K_mat)
        else:
            ...
            concat = tf.keras.layers.Concatenate(axis=-1, name="concat_k_seg")(
                [x, segment, K_mat]
            )
